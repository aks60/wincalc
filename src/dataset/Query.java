    ...
    public boolean delete(Record record) {
        try {
            ...
            if (e.getErrorCode() == 335544466) {
                JOptionPane.showMessageDialog(eProfile.frame, "Нельзя удалить запись на которую имеются ссылки из других форм", "SQL предупреждение", JOptionPane.INFORMATION_MESSAGE);
                return false;
            }
        }
        return true;
            ...
            for (Record record : this) {
                ...
                if (record.get(0).equals(Query.UPD) || record.get(0).equals(INS)) {
                    ...
                    if (record.validateRec(fields) != null) { //проверка на корректность ввода данных                        
                        JOptionPane.showMessageDialog(eProfile.frame, "Таблица <" + fields.get(0).fields()[0].meta().descr + ">.\n" + message
