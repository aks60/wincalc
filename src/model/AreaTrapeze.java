...
/**
 * Является родителем для всех элементов образуюших трапецию
 * Контролирует и управляет поведение элементов трапеции
 */
public class AreaTrapeze extends AreaSimple {

    public AreaTrapeze(IWindows iwin, String id, LayoutArea layout, float width, float height, int colorBase, int colorInterna, int colorExternal, String paramJson) {
    ...
    public void setSpecifElement(ElemFrame elemFrame, Sysproa sysproaRec) {
        ...
        /*if(LayoutArea.RIGHT == elemFrame.getLayout()) {
            elemFrame.y1 = getIwin().getHeightAdd() - getIwin().getHeight();
            elemFrame.width = elemFrame.y2 - elemFrame.y1;
            if(this != null) System.out.println("ТЕСТОВАЯ ЗАПЛАТКА");
        }*/
        //iwin.heightAdd = iwin.height;
    }
